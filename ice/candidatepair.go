...
func (p *candidatePair) Write(b []byte) (int, error) {
	...
	udpMsgLen := len(b)
	tcpMsg := make([]byte, udpMsgLen + 2)
	binary.BigEndian.PutUint16(tcpMsg, uint16(udpMsgLen))
	copy(tcpMsg[2:], b)
	// a.log.Debugf("Write %d bytes", len(tcpMsg))

	return a.IceConn.Write(tcpMsg)
}
...
func (a *Agent) sendSTUN(msg *stun.Message, local, remote *Candidate) {
	...
	udpMsg := msg.Pack()
	udpMsgLen := len(udpMsg)
	tcpMsg := make([]byte, udpMsgLen + 2)
	binary.BigEndian.PutUint16(tcpMsg, uint16(udpMsgLen))
	copy(tcpMsg[2:], udpMsg)
	
	n, err := a.IceConn.Write(tcpMsg)
	if err != nil {
		a.log.Tracef("failed to send STUN message: %s", err)
	}
	local.seen(true)
	
	a.log.Debugf("sendSTUN %d bytes", n)
}
